...
### 🎯 **AI 어시스턴트 담당 영역**
- **GCP 인프라**: Terraform/Ansible을 통한 자동화 작업
- **Ansible 설정**: 모든 환경의 플레이북 및 역할 구성
- **문서화**: 가이드 작성 및 진행 상황 추적
- **AWS 정보 제공**: CLI 명령어를 통한 상태 확인 및 가이드 제공

### 👨‍💻 **사용자 직접 작업 영역**  
- **AWS 콘솔 작업**: EC2, VPC, 보안 그룹 등 모든 AWS 리소스 직접 구성
- **AWS 학습**: 실습을 통한 AWS 서비스 이해도 향상
- **설정 검증**: AWS CLI를 통한 구성 확인

### 🤝 **협업 방식**
- AI가 AWS CLI 명령어로 현재 상태 파악 및 가이드 제공
- 사용자가 AWS 콘솔에서 직접 리소스 생성/구성
- AI가 Ansible을 통해 생성된 AWS 리소스에 애플리케이션 배포
